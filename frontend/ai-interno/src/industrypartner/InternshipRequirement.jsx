...
export default function InternshipRequirements() {
  ...
  const navigate = useNavigate();
  const location = useLocation(); // Add useLocation to detect route changes
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://localhost:5000/api/industry-partner/internships',
        {
          ...formData,
          skillInternWillLearn: formData.skills,
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-white relative">
              ...
              <div className="p-4">
                ...
                <div className="flex justify-end">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDeleteInternship(req._id);
                    }}
                    className="text-red-500 hover:text-red-700"
                  >
      ...
      <div
        className={`fixed inset-y-0 right-0 w-full max-w-lg bg-white shadow-xl transform transition-transform duration-300 ease-in-out z-50 ${
