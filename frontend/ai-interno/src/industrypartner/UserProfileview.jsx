...
import { useParams } from "react-router-dom";  // Import useParams to access userId from URL
import {
  User,
  Badge,
  Briefcase,
...
const UserProfileView = () => {
  // Access userId from URL using useParams
  const { userId } = useParams();  // Getting userId from the URL
  console.log('User ID from URL:', userId);  // Log the userId for debugging
  ...
  useEffect(() => {
    if (!userId) {
      // If no userId found in the URL, handle this scenario
    ...
    const fetchData = async () => {
      try {
        // Fetching data based on userId from the URL
        const userResponse = await axios.get(`/api/user/${userId}`);
        const userDetailsResponse = await axios.get(`/api/user-details/${userId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="bg-gradient-to-r from-teal-600 to-teal-600 rounded-lg shadow-lg p-6 sm:p-10 text-white">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
              ...
              <div className="text-center md:text-left">
                ...
                <div className="flex flex-wrap gap-2 mt-3 justify-center md:justify-start">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full ${userData.status === "Active" ? "bg-green-500/20" : "bg-red-500/20"} text-sm`}
                  >
                    {userData.status}
                  </span>
                  <span className="px-3 py-1 rounded-full bg-white/20 text-sm">
